...
class FormFieldsGenerator
{
    ...
    public static function text($templateData, $field)
    {
        ...
        $textField .= "\n\t{!! Form::textField('','\$FIELD_NAME\$', null, [" . $validatorInput . "'class' => 'form-control', 'placeholder' => '\$FIELD_NAME\$']) !!}";
    ...
    public static function textarea($templateData, $field)
    {
        ...
        $textareaField .= "\n\t{!! Form::textarea('\$FIELD_NAME\$', null, [" . $validatorInput . "'class' => 'form-control', 'placeholder' => '\$FIELD_NAME\$']) !!}";
    ...
    public static function email($templateData, $field)
    {
        ...
        $textField .= "\n\t{!! Form::email('\$FIELD_NAME\$', null, [" . $validatorInput . "'class' => 'form-control', 'placeholder'=> '\$FIELD_NAME\$']) !!}";
        $templateData = str_replace('$FIELD_INPUT$', $textField, $templateData);
    ...
    public static function number($templateData, $field)
    {
        ...
        $textField .= "\n\t{!! Form::numberField('','\$FIELD_NAME\$', null, ['class' => 'form-control', 'placeholder' => '\$FIELD_NAME\$']) !!}";
        $templateData = str_replace('$FIELD_INPUT$', $textField, $templateData);
    ...
    public static function date2($templateData, $field)
    {
        ...
        $textField = '';
        $textField .= "\n\t{!! Form::dateField('','\$FIELD_NAME\$', null, ['datetime' => ['locale' => 'pt'],'class' => 'form-control date-picker', 'placeholder' => '\$FIELD_NAME\$']) !!}";
        $templateData = str_replace('$FIELD_INPUT$', $textField, $templateData);
    ...
    public static function select2($templateData, $field, $inputArray = true, $modelName = null)
    {
        ...
        $validatorInput = self::getInputValidators($field);
        //Form::select2Field('Select2 Async Multiple', 'select2-async-multiple', [], [2, 3], ['select2' => ['ajax--url' => '/select2/data'], 'multiple' => true])
        $textField .= "\n\t{!! Form::select2Field('\$FIELD_NAME\$','\$FIELD_NAME\$', \$INPUT_ARR\$, isset($\$MODEL_NAME\$) ? [$\$MODEL_NAME\$->\$FIELD_NAME\$]: null , ['select2' => ['ajax--url' => '\$URL_DATA\$'], " . $validatorInput . "'class' => 'form-control js-data-example-ajax', , 'placeholder' => '\$FIELD_NAME\$']) !!}";
